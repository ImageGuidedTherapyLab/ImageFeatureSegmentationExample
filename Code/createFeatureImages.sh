...
################################################################################
#
# Normalize images -> truncate intensity and denoise
#
################################################################################

for (( i = 0; i < ${#ANATOMICAL_IMAGES[@]}; i++ ))
  do
    OUTPUT_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_DENOISE.${OUTPUT_SUFFIX}
    RAWIMAGE_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_RAWIMAGE.${OUTPUT_SUFFIX}
    RESCALE_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_RESCALE.${OUTPUT_SUFFIX}
    GRADIENT_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_GRADIENT.${OUTPUT_SUFFIX}
...
#        logCmd ${ANTSPATH}ImageMath 3 $OUTPUT_IMAGE HistogramMatch $OUTPUT_IMAGE ${SYMMETRIC_TEMPLATES[$i]}  200 12
    ...
    # create png
    PNG_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}.png
    if [[ ! -f ${PNG_IMAGE} ]];
      then
        logCmd c3d ${ANATOMICAL_IMAGES[$i]} -slice z $CENTROIDSLICE -clip 0 200 -pad 1x0x0vox 0x0x0vox 200 -color-map grey -type uchar -omc $PNG_IMAGE
      fi

    # create png
    PNG_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_DENOISE.png
    if [[ ! -f ${PNG_IMAGE} ]];
      then
        logCmd c3d $OUTPUT_IMAGE  -slice z $CENTROIDSLICE -clip 0 200 -pad 1x0x0vox 0x0x0vox 200 -color-map grey -type uchar -omc $PNG_IMAGE
      fi

    # compute image gradient
    if [[ ! -f ${GRADIENT_IMAGE} ]];
      then
        logCmd ${ANTSPATH}GradientMagnitudeImageFilter $OUTPUT_IMAGE $GRADIENT_IMAGE
      fi
    # create png
    PNG_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_GRADIENT.png
    if [[ ! -f ${PNG_IMAGE} ]];
      then
        logCmd c3d $GRADIENT_IMAGE -slice z $CENTROIDSLICE -clip 0 25 -pad 1x0x0vox 0x0x0vox 25 -color-map jet -type uchar -omc $PNG_IMAGE
      fi

    if [[ ! -f ${RESCALE_IMAGE} ]];
      then
        logCmd ${ANTSPATH}ImageMath 3 $RESCALE_IMAGE RescaleImage $OUTPUT_IMAGE 0 1
      fi
  done
  ...
  do
    ...
    for (( j = 0; j < ${#RADII[@]}; j++ ))
      do
        ...
        # create png
        PNG_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_MEAN_RADIUS_${RADII[$j]}.png
        if [[ ! -f ${PNG_IMAGE} ]];
          then
             logCmd c3d $OUTPUT_IMAGE -slice z $CENTROIDSLICE -clip 0 200 -pad 1x0x0vox 0x0x0vox 200  -color-map grey -type uchar -omc $PNG_IMAGE
          fi
        ...
        PNG_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_SIGMA_RADIUS_${RADII[$j]}.png
        if [[ ! -f ${PNG_IMAGE} ]];
          then
             logCmd c3d $OUTPUT_IMAGE -slice z $CENTROIDSLICE -clip 0 25 -pad 1x0x0vox 0x0x0vox 25  -color-map grey -type uchar -omc $PNG_IMAGE
          fi
...
OUTPUT_DISTANCE_IMAGE=${OUTPUT_PREFIX}NORMALIZED_DISTANCE.${OUTPUT_SUFFIX}
if [[ ! -f ${OUTPUT_DISTANCE_IMAGE} ]];
  then
    logCmd ${ANTSPATH}ImageMath ${DIMENSION} $OUTPUT_DISTANCE_IMAGE MaurerDistance $MASK_IMAGE 0
    logCmd ${ANTSPATH}ImageMath ${DIMENSION} $OUTPUT_DISTANCE_IMAGE m $MASK_IMAGE $OUTPUT_DISTANCE_IMAGE
    logCmd ${ANTSPATH}ImageMath ${DIMENSION} $OUTPUT_DISTANCE_IMAGE Normalize $OUTPUT_DISTANCE_IMAGE
  fi
# create png
      ...
      then
        ...
        if [[ ! -f ${OUTPUT_ATROPOS_IMAGE} ]];
          then
            ...
            ATROPOSN4CMD="bash ${ANTSPATH}/antsAtroposN4.sh \
              -d ${DIMENSION} \
              -b Socrates[0] \
              ...
              -t [10x10x10x10,0] \
              -s ${OUTPUT_SUFFIX}"
            echo "bias correction needed ? " $ATROPOSN4CMD
            # $ATROPOSN4CMD
            ...
            #     logCmd mv $f $newfile
            #   done
            echo "Run ATROPOS only..." 
            SEG_BASE="${ANTSPATH}/Atropos -d ${DIMENSION} -a ${NORMALIZED_IMAGES[$i]}"
            SEG_0="-i PriorProbabilityImages[${NUMBER_OF_LABELS},${SEGMENTATION_PRIOR},0.0] -p Socrates[0] -x $MASK_IMAGE"
            SEG_1="-c [5,0] -k Gaussian -m [0.1,1x1x1]"
        ...
        if [[ ! -f ${OUTPUT_ATROPOS_IMAGE} ]];
          then
            logCmd echo "-c [0,0] impose no spatial regularization ??"
            logCmd $SEG_BASE $SEG_0 $SEG_1 $SEG_2
          fi
        #echo $SEG_BASE $SEG_0 $SEG_1 $SEG_2

        OUTPUT_ATROPOS_FEATURES_PREFIX=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_ATROPOS_GMM_
        OUTPUT_ATROPOS_DISTANCE_IMAGE=${OUTPUT_ATROPOS_FEATURES_PREFIX}LABEL${CORE_LABEL}_DISTANCE.${OUTPUT_SUFFIX}
        OUTPUT_ATROPOS_CANNY_IMAGE=${OUTPUT_ATROPOS_FEATURES_PREFIX}CANNY.${OUTPUT_SUFFIX}

        #echo ${ANTSPATH}/GetConnectedComponentsFeatureImages ${DIMENSION} ${OUTPUT_ATROPOS_IMAGE} ${OUTPUT_ATROPOS_FEATURES_PREFIX}
        #echo ${ANTSPATH}CannyEdgeDetectionImageFilter ${OUTPUT_ATROPOS_IMAGE} ${OUTPUT_ATROPOS_CANNY_IMAGE}
        #echo ${ANTSPATH}/ImageMath ${DIMENSION} ${OUTPUT_ATROPOS_CANNY_IMAGE} MaurerDistance ${OUTPUT_ATROPOS_CANNY_IMAGE} 1
        if [[ ! -f ${OUTPUT_ATROPOS_FEATURES_PREFIX}ECCENTRICITY.nii.gz ]];
          then
            logCmd ${ANTSPATH}CannyEdgeDetectionImageFilter ${OUTPUT_ATROPOS_IMAGE} ${OUTPUT_ATROPOS_CANNY_IMAGE}
            logCmd ${ANTSPATH}/GetConnectedComponentsFeatureImages ${DIMENSION} ${OUTPUT_ATROPOS_IMAGE} ${OUTPUT_ATROPOS_FEATURES_PREFIX}
            logCmd echo ${ANTSPATH}/ThresholdImage ${DIMENSION} ${OUTPUT_ATROPOS_IMAGE} ${OUTPUT_ATROPOS_DISTANCE_IMAGE} ${CORE_LABEL} ${CORE_LABEL} 1 0
            logCmd c3d ${OUTPUT_ATROPOS_IMAGE} -thresh ${CORE_LABEL} ${CORE_LABEL} 1 0 ${OUTPUT_DISTANCE_IMAGE} -thresh .17 inf 1 0 -multiply -comp -thresh 1 1 1 0  -o ${OUTPUT_ATROPOS_DISTANCE_IMAGE} 
            #logCmd c3d ${OUTPUT_ATROPOS_IMAGE} -thresh ${CORE_LABEL} ${CORE_LABEL} 1 0 -comp -thresh 1 1 1 0 -holefill 1 0 ${OUTPUT_ATROPOS_DISTANCE_IMAGE} 
            logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${OUTPUT_ATROPOS_DISTANCE_IMAGE} MaurerDistance ${OUTPUT_ATROPOS_DISTANCE_IMAGE} 1
            logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${OUTPUT_ATROPOS_CANNY_IMAGE} MaurerDistance ${OUTPUT_ATROPOS_CANNY_IMAGE} 1
          fi
        # create png
        PNG_IMAGE=${OUTPUT_PREFIX}${IMAGE_NAMES[$i]}_ATROPOS_GMM_CANNY.png
        if [[ ! -f ${PNG_IMAGE} ]];
          then
