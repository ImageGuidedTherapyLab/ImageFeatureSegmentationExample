...
SHELL=/bin/bash
#SCRIPTSPATH=/data/fuentes/NCT00548756/BRATS2013/Scripts

if [[ ! -d "$SCRIPTSPATH" ]];
  then
    echo error: need to change so that SCRIPTSPATH=$SCRIPTSPATH is where createFeatureImages.sh
    exit 1
  fi
...
echo ${SHELL} ${SCRIPTSPATH}/createFeatureImages.sh ${COMMAND_LINE}
${SHELL} ${SCRIPTSPATH}/createFeatureImages.sh ${COMMAND_LINE}
...
echo  "Rscript ${SCRIPTSPATH}/createCSVFileFromModel.R ${MODEL} ${MASK_IMAGE} ${OUTPUT_PREFIX} ${CSV_FILE}"
logCmd Rscript ${SCRIPTSPATH}/createCSVFileFromModel.R ${MODEL} ${MASK_IMAGE} ${OUTPUT_PREFIX} ${CSV_FILE}

echo "######################################################################################"
#
echo "Apply the model"
#
echo "######################################################################################"

echo   Rscript ${SCRIPTSPATH}/applyModel.R ${DIMENSION} ${MODEL} ${CSV_FILE} ${KFOLDOUTPUT}RF_POSTERIORS 1
logCmd Rscript ${SCRIPTSPATH}/applyModel.R ${DIMENSION} ${MODEL} ${CSV_FILE} ${KFOLDOUTPUT}RF_POSTERIORS 1
